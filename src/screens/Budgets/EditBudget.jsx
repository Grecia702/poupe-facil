import { StyleSheet, Text, TextInput, TouchableOpacity, View, Modal, FlatList, ScrollView } from 'react-native'
...
const CreateBudget = () => {
    ...
    const { refetchBudget, updateBudgetMutation } = useBudgetAuth()
    ...
    const [fields, setFields] = useState(
        {
            quantia_limite: Number(data?.limite) || 0,
            limites_categorias: data?.limites_categorias
        });
    const [categoriasLimites, setCategoriasLimites] = useState(new Set(Object.keys(fields.limites_categorias || {})));
    ...
    const toastSuccess = () => {
        toast.show('Orçamento editado com sucesso', {
            type: 'success',
            duration: 2500,
        });
        setTimeout(() => {
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: isDarkMode ? "#333" : '#ffffff' }]}>
            <CustomInput
