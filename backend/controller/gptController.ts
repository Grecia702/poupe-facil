...
import { getActiveService } from '../services/budgetServices.ts'
...
`
        ...
        if (parsed.command === 'create') {
            ...
            const valorTransacoes = queryResult.reduce(
            ...
            const createdMessage = queryResult
                .map((row) => {
...
`.trim();
            ...
            let baseContent = `${queryResult.length > 1 ? 'Transações adicionadas' : 'Transação adicionada'}:\n\n${createdMessage}`;
